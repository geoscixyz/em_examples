...
indy = (
    (mesh.gridFy[:, 0] >= xmin) & (mesh.gridFy[:, 0] <= xmax) &
...
def calculateRhoA(survey, VM, VN, A, B, M, N):
    ...
    if(survey == "Dipole-Dipole"):
        G = (
            1. / (
                1./(np.abs(A-M)+eps) - 1./(np.abs(M-B)+eps) -
...
def plot_Surface_Potentials(
    survey, A, B, M, N, zcLayer, dzLayer, xc, zc, r, rhoHalf, rhoLayer,
    rhoTarget, Field, Type, Scale
):
    labelsize = 12.
    ticksize = 10.
    ...
    ax[0].plot(
        xSurface, phiPrimSurface, linestyle='dashed', linewidth=0.5, color='k'
    )
    ax[0].grid(
        which='both', linestyle='-', linewidth=0.5, color=[0.2, 0.2, 0.2],
        alpha=0.5
    )

    if(survey == "Pole-Dipole" or survey == "Pole-Pole"):
        ax[0].plot(
            A, 0, '+', markersize=12, markeredgewidth=3, color=[1., 0., 0]
        )
    else:
        ax[0].plot(
            A, 0, '+', markersize=12, markeredgewidth=3, color=[1., 0., 0]
        ...
        ax[0].plot(
            B, 0, '_', markersize=12, markeredgewidth=3, color=[0., 0., 1.]
        )
    ...
    if(survey == "Dipole-Pole" or survey == "Pole-Pole"):
        ...
        ax[0].annotate(
            '%2.1e' % (VN), xy=xytextN, xytext=xytextN, fontsize=labelsize
        )
    ...
    if(rhoTarget != rhoHalf):
        ax[1].plot(
            cylinderPoints[:, 0], cylinderPoints[:, 1],
    ...
    if (Field == 'Charge') and (Type != 'Primary') and (Type != 'Total'):
        ...
        ax[1].plot(
            qPosAvgLoc[0], qPosAvgLoc[1], marker='.', color='black',
            markersize=labelsize
        )
        ax[1].plot(
            qNegAvgLoc[0], qNegAvgLoc[1], marker='.',  color='black',
            markersize=labelsize
        )
        ...
        ax[1].annotate(
            '+Q = %2.1e' % (qPosSum), xy=xytext_qPos, xytext=xytext_qPos,
            fontsize=labelsize
        )
        ax[1].annotate(
            '-Q = %2.1e' % (qNegSum), xy=xytext_qNeg, xytext=xytext_qNeg,
            fontsize=labelsize
        )
    ...
    if Scale == 'Log':
        ...
        if (Field == 'E') or (Field == 'J'):
            cb = plt.colorbar(
                dat[0], ax=cbar_ax, format=formatter,
            ...
            if (Type == 'Secondary'):
                cb = plt.colorbar(
                    dat[0], ax=cbar_ax, format=formatter,
                    ticks=np.r_[np.minimum(0., vmin), np.maximum(0., vmax)]
                )
            else:
                cb = plt.colorbar(
                    dat[0], ax=cbar_ax, format=formatter,
            ...
            cb = plt.colorbar(
                ...
                ticks=np.r_[
                    -1.*np.logspace(
                        np.log10(-vmin-eps), np.log10(linthresh), 3
                    )[:-1],
                    0.,
